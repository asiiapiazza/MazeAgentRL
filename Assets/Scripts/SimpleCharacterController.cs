...
public class SimpleCharacterController : MonoBehaviour
{
    ...
    private void ProcessActions()
    {
        ...
        if (IsGrounded)
        {
            ...
            if (JumpInput && allowJump && !jumpPressed)
            {
                // Apply an upward velocity to jump
                rigidbody.linearVelocity += Vector3.up * jumpSpeed;
                jumpPressed = true; // Imposta lo stato del salto a vero
            }

            // Apply a forward or backward velocity based on player input
            rigidbody.linearVelocity += transform.forward * Mathf.Clamp(ForwardInput, -1f, 1f) * moveSpeed;
        }
        else
        {
            //// Apply gravity
            //rigidbody.linearVelocity += Vector3.up * -9.18f * Time.fixedDeltaTime;
            ...
            if (!Mathf.Approximately(ForwardInput, 0f))
            {
                ...
                rigidbody.linearVelocity = verticalVelocity + transform.forward * Mathf.Clamp(ForwardInput, -1f, 1f) * moveSpeed / 2f;
            }
        }

        // Reset jumpPressed when the jump key is released
        if (!JumpInput)
        {
            jumpPressed = false;
        }
